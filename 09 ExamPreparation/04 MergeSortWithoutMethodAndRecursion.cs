...
class MergeSortAlg
{
    static void Main()
    {
        ...
        {
            //A loop that go over the elements of the initial array. The step, at which i increases depends on the variable currentLenght,
            //so at the first cycle of the while loop we compare adjacent elements, then the first elements of adjacent pairs, then the first 
            //elements of adjacent quadruplets etc.
            for (int i = 0; i < arraySize; i = i + currentLength * 2)
            {
                while (currentPosition < arraySize)
                {
                    ...
                    //3) the position of the element to be compared in the right array is not outside of the initial array (in case of hanging elements,
                    //when the size of the initial array is not a power of 2
                    if (left < currentLength && right < currentLength && i + right + currentLength < arraySize)
                    {
                        ...
                        //this if condition compares the first elements of the left and right arrays. Whichever element is slower is added as the next in
                        //line to the help array and the counter of the respective subarray (left or right) is increased by 1
                    ...
                    //if none of the conditions is fulfilled, there are no elements to be compered in eighter of the two subarrays and the next cycle
                    //of the outer while loop must be executed
                    else
                    {
                        break;
                    }
